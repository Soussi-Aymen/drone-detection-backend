...
export class SimulationService implements OnModuleInit {
  ...
  constructor(
    ...
    const defaultLat = parseFloat(
      process.env.SYSTEM_DEFAULT_LATITUDE || "52.5200",
    );
    const defaultLng = parseFloat(
      process.env.SYSTEM_DEFAULT_LONGITUDE || "13.4050",
    );
  ...
  onModuleInit() {
    ...
    console.log(
      `[SIM] Radar warm-up phase initiated. Threat spawning will begin in ${this.WARMUP_DELAY_MS / 1000} seconds.`,
    );
    setTimeout(() => {
  ...
  private updateThreatTrack() {
    ...
    if (
      this.threatTrack.distance > this.MAX_DETECTION_RANGE_M * 1.5 ||
      this.threatTrack.confidence < 10
    ) {
      // Drone is too far or confidence dropped, clear track and prepare for new one
      console.log(
        `Track ${this.threatTrack.trackId} expired or out of range. Cleaning up.`,
      );
    ...
    if (Math.random() < 0.02) {
      // 2% chance per update (0.02 * 5 updates/sec = 10% chance per second)
      this.threatTrack.bearing =
        (this.threatTrack.bearing + (Math.random() * 90 - 45) + 360) % 360;
    }
  }
  ...
  private createNewThreat(): IThreatTrack {
    ...
    const randomDistance =
      Math.random() * this.THREAT_SPAWN_RANGE_M * 0.8 + 500; // 500m to 3600m
    ...
    console.log(
      `Spawning new drone track ${this.trackIdCounter} at ${Math.round(randomDistance)}m.`,
    );
  ...
  private recalculateRelativeMetrics() {
    ...
    if (distanceM > this.MAX_DETECTION_RANGE_M * 0.9) {
      this.threatTrack.confidence = Math.max(
        0,
        this.threatTrack.confidence - 1,
      );
    } else if (this.threatTrack.confidence < 100) {
      this.threatTrack.confidence = Math.min(
        100,
        this.threatTrack.confidence + 1,
      );
    }
